...
local panelMenu = {
	{"Nowe", tocolor(217, 83, 79), "new"},
	{"Cofnij", tocolor(240, 173, 78, 230), "undo"},
	{"Popraw", tocolor(240, 173, 78, 230), "redo"},
	{"Zapisz", tocolor(21, 219, 114), "save"},
	...
	{"Grubość pędzla", tocolor(50, 179, 239, 230), "brushsize", false,
		activeWhen = function()
	...
	{"Tło (ciemne/jasne)", tocolor(50, 179, 239, 230), "theme"},
	{"Zapisane graffiti", tocolor(21, 219, 114), "savedgraffitis", 10,
		activeWhen = function()
	...
	{"Wczytaj obrazek z URL", tocolor(244, 184, 235), "url_image", 11,
		activeWhen = function()
...
addEvent("onPlayerEnableGraffiti", true)
addEventHandler("onPlayerEnableGraffiti", getRootElement(),
	function ()
	showTheEditor()
	end)
...
addCommandHandler("gsize",
	function (command, size)
		if not tonumber(size) or tonumber(size) <= 0 or tonumber(size) > 5 then
			...
			outputChatBox("#b0e56b[Graffiti]:#ffffff Zmieniono rozmar graffiti na #e56b6b" .. graffitiSize .. ".", 255, 255, 255, true)
		end
	end
)


addEvent("onAdminOpenGraffitiEditor", true)
addEventHandler("onAdminOpenGraffitiEditor", getRootElement(),
	function ()
...
function onClientCursorMovee(relativeX, relativeY, absoluteX, absoluteY)
...
function onLocalClick(button, state)
...
-- addEventHandler("onClientClick", getRootElement, onLocalClick)
				...
				if deltaTime >= 3000 then
					if utf8.len(path) > 0 then
						...
						if fileExtension and fileExtension == ".png" or fileExtension == ".jpg" or fileExtension == ".jpeg" or fileExtension == ".bmp" then
							...
							outputChatBox("#32b3ef[Graffiti]: #ffffffSciaganie graffiti!", 255, 255, 255, true)
						else
							outputChatBox("#e56b6b[Graffiti]: #ffffffZly format obrazka", 255, 255, 255, true)
						end
					else
						outputChatBox("#e56b6b[Graffiti]: #ffffffDodaj link do pobrania!", 255, 255, 255, true)
					end
				else
					outputChatBox("#e56b6b[Graffiti]: #ffffffPoczekaj #e56b6b" .. 3 - math.floor(deltaTime / 1000) .. " sekund#ffffff przed następnym pobraniem.", 255, 255, 255, true)
				end
			else
				outputChatBox("#e56b6b[Graffiti]: #ffffffJuż sciąga się jeden obrazek!", 255, 255, 255, true)
			end
		end
	end
)
...
addEventHandler("onClientReceiveDownloadedImage", getRootElement(),
	function (imagePixels, errno)
		if errno == 0 then
			...
			outputChatBox("#32b3ef[Graffiti]: #ffffffZapisano graffiti.", 255, 255, 255, true)
			outputChatBox("#32b3ef[Graffiti]: #ffffffMożesz znależć je w 'Zapisane graffiti'.", 255, 255, 255, true)
		else
			outputChatBox("#e56b6b[Graffiti]: #ffffffBłąd ze znalezieniem graffiti.", 255, 255, 255, true)
		end
...
function localMinimize()
if drawingSurface and isElement(drawingSurface) then
			lastDrawedPixels = dxGetTexturePixels(drawingSurface)
		end
end
-- addEventHandler("onClientMinimize". getRootElement(), localMinimize)

function localRestore(didClearRenderTargets)
...
function onClientPlayerFire(weapon, _, _, hitX, hitY, hitZ)
if weapon ~= 41 then
			return
		end
...
-- addEventHandler("onClientPlayerWeaponFire", localPlayer, onClientPlayerFire)
...
function anotherClick(button, state)
...
-- addEventHandler("onClientClick", getRootElement(), anotherClick)
...
function showTheEditor()
	if isEventHandlerAdded("onClientRender", getRootElement(), renderTheEditor) then
		...
		-- removeEventHandler("onClientPlayerWeaponFire", localPlayer, onClientPlayerFire)
		-- removeEventHandler("onClientCursorMove", getRootElement(), onClientCursorMovee)
		-- removeEventHandler("onClientClick", getRootElement(), onLocalClick)
		-- removeEventHandler("onClientRestore", getRootElement(), localRestore)
		-- removeEventHandler("onClientMinimize", getRootElement(), localMinimize)
		-- removeEventHandler("onClientClick", getRootElement(), anotherClick)
			...
			elseif string.find(selectedButton, "menu|") then
				...
				if savedGraffitisMenu.isActive then
					if string.find(selectedButton, "select|") then
						...
						promptDialog.text = "Czy na pewno edytujesz wybrane graffiti?\nTa czynność nadpisuje bieżący rysunek na powierzchni rysunku."
						promptDialog.value = selectedButton
					elseif string.find(selectedButton, "remove|") then
						promptDialog.isActive = true
						promptDialog.text = "Jesteś pewny że chcesz usunąć to graffiti?\nTa akcja nie może zostać cofnięta."
						promptDialog.value = selectedButton
					end
				else
...
function getColorFromDecimal(decimal)
	...
	return red, green, blue, alpha
		...
		addEventHandler("onClientCursorMove", getRootElement(), onClientCursorMovee)
		addEventHandler("onClientClick", getRootElement(), onLocalClick)
		addEventHandler("onClientRestore", getRootElement(), localRestore)
		addEventHandler("onClientMinimize", getRootElement(), localMinimize)
		addEventHandler("onClientClick", getRootElement(), anotherClick)
