...
colorPicker = {
	...
	constructor = function(info)
		...
		colorPicker.GUI.selectWindow = guiCreateWindow(screenW - info.selectWindow.width,
			(screenH - info.selectWindow.height) / 2,
		...
		colorPicker.GUI.palette = guiCreateStaticImage(colorPicker.selectWindow.paletteX,
			colorPicker.selectWindow.paletteY,
			256, 256, "client/colorpicker/palette.png", false, colorPicker.GUI.selectWindow)
		colorPicker.GUI.alphaBar = guiCreateStaticImage(
			colorPicker.selectWindow.paletteX + 255 + colorPicker.selectWindow.alphaOffset,
			colorPicker.selectWindow.paletteY,
			colorPicker.selectWindow.alphaWidth, 255, "client/colorpicker/alpha.png", false, colorPicker.GUI
			.selectWindow)
		colorPicker.isSelectOpen = false

		-- Create the RGB and HSL edit boxes
		colorPicker.children.R = guiCreateEdit(info.selectWindow.rgbX + 10, info.selectWindow.rgbY,
			info.selectWindow.rgbWidth, info.selectWindow.rgbHeight, tostring(colorPicker.value[1]), false,
			colorPicker.GUI.selectWindow)
		...
		colorPicker.children.G = guiCreateEdit(info.selectWindow.rgbX + 10,
			info.selectWindow.rgbY + info.selectWindow.rgbHeight, info.selectWindow.rgbWidth, info.selectWindow
			.rgbHeight, tostring(colorPicker.value[2]), false, colorPicker.GUI.selectWindow)
		...
		colorPicker.GUI.labelG = guiCreateLabel(info.selectWindow.rgbX,
			info.selectWindow.rgbY + 3 + info.selectWindow.rgbHeight,
			10, 20, "G", false, colorPicker.GUI.selectWindow)
		guiSetFont(colorPicker.GUI.labelG, "default-bold-small")

		colorPicker.children.B = guiCreateEdit(info.selectWindow.rgbX + 10,
			info.selectWindow.rgbY + info.selectWindow.rgbHeight * 2, info.selectWindow.rgbWidth,
			info.selectWindow.rgbHeight, tostring(colorPicker.value[3]), false, colorPicker.GUI.selectWindow)
		...
		colorPicker.GUI.labelB = guiCreateLabel(info.selectWindow.rgbX,
			info.selectWindow.rgbY + 3 + info.selectWindow.rgbHeight * 2,
			10, 20, "B", false, colorPicker.GUI.selectWindow)
		guiSetFont(colorPicker.GUI.labelB, "default-bold-small")

		colorPicker.children.A = guiCreateEdit(info.selectWindow.rgbX + 10,
			info.selectWindow.rgbY + info.selectWindow.rgbHeight * 3, info.selectWindow.rgbWidth,
			info.selectWindow.rgbHeight, tostring(colorPicker.value[4]), false, colorPicker.GUI.selectWindow)
		...
		colorPicker.GUI.labelA = guiCreateLabel(info.selectWindow.rgbX - 25,
			info.selectWindow.rgbY + 3 + info.selectWindow.rgbHeight * 3,
		...
		colorPicker.children.Hex = guiCreateEdit(info.selectWindow.rgbX - 15,
			info.selectWindow.rgbY + info.selectWindow.rgbHeight * 4,
			info.selectWindow.rgbWidth * 1.5, info.selectWindow.rgbHeight, "#" .. string.format("%02X%02X%02X%02X",
				colorPicker.value[1], colorPicker.value[2], colorPicker.value[3], colorPicker.value[4]), false,
			colorPicker.GUI.selectWindow)
		guiEditSetMaxLength(colorPicker.children.Hex, 9)

		-- Add event handlers for Hex input
		addEventHandler("onClientGUIChanged", colorPicker.children.Hex, colorPicker.forceNaturalAndRangeHex)
		addEventHandler("onClientGUIChanged", colorPicker.children.Hex, colorPicker.selectionManualInputHex)
		-- Create the "Hex" label
		colorPicker.GUI.labelHex = guiCreateLabel(info.selectWindow.rgbX - 43,
			info.selectWindow.rgbY + 3 + info.selectWindow.rgbHeight * 4,
			50, 20, "Hex", false, colorPicker.GUI.selectWindow)
		guiSetFont(colorPicker.GUI.labelHex, "default-bold-small")

		--------------------------------------------------------------------------------------- HEX !!!!!!!!!!!!!!!!!!!!!!!!


		colorPicker.h, colorPicker.s, colorPicker.l = colorPicker.rgb2hsl(colorPicker.value[1] / 255,
			colorPicker.value[2] / 255, colorPicker.value[3] / 255)

		colorPicker.children.H = guiCreateEdit(info.selectWindow.hslX + 10, info.selectWindow.hslY,
			info.selectWindow.hslWidth, info.selectWindow.hslHeight, tostring(math.floor(colorPicker.h * 255)), false,
			colorPicker.GUI.selectWindow)
		...
		colorPicker.children.S = guiCreateEdit(info.selectWindow.hslX + 10,
			info.selectWindow.hslY + info.selectWindow.hslHeight, info.selectWindow.hslWidth, info.selectWindow
			.hslHeight, tostring(math.floor(colorPicker.s * 255)), false, colorPicker.GUI.selectWindow)
		...
		colorPicker.GUI.labelS = guiCreateLabel(info.selectWindow.hslX,
			info.selectWindow.hslY + 3 + info.selectWindow.hslHeight,
			10, 20, "S", false, colorPicker.GUI.selectWindow)
		guiSetFont(colorPicker.GUI.labelS, "default-bold-small")

		colorPicker.children.L = guiCreateEdit(info.selectWindow.hslX + 10,
			info.selectWindow.hslY + info.selectWindow.hslHeight * 2, info.selectWindow.hslWidth,
			info.selectWindow.hslHeight, tostring(math.floor(colorPicker.s * 255)), false, colorPicker.GUI.selectWindow)
		...
		colorPicker.GUI.labelL = guiCreateLabel(info.selectWindow.hslX,
			info.selectWindow.hslY + 3 + info.selectWindow.hslHeight * 2,
	...
	forceNaturalAndRangeHex = function()
		local inputText = guiGetText(source)
		local changedText = inputText:upper():gsub("[^#%x]", "")
		if changedText:find("#", 2) then
			changedText = "#" .. changedText:gsub("#", "")
		end
		if #changedText > 7 then
			changedText = changedText:sub(1, 7)
		end
		if changedText ~= inputText then
			guiSetText(source, changedText)
		end
	...
	selectionManualInputHex = function()
		if not colorPicker.avoidRecursion then
			colorPicker.avoidRecursion = true
			local hex = guiGetText(colorPicker.children.Hex)
			local r, g, b = hexToRgba(hex)
			...
			colorPicker.h, colorPicker.s, colorPicker.l = colorPicker.rgb2hsl(r / 255, g / 255, b / 255)
			colorPicker.setValue({ r, g, b, colorPicker.value[4] })
			colorPicker.avoidRecursion = false
		end
	end,
	...
	updateSelectionWindowEdits = function()
		...
		guiSetText(colorPicker.children.Hex,
			tostring(rgbaToHex(colorPicker.value[1], colorPicker.value[2], colorPicker.value[3], colorPicker.value[4])))
	end,
	updateTempColors = function()
		r = colorPicker.value[1]
		g = colorPicker.value[2]
		b = colorPicker.value[3]
		a = colorPicker.value[4]
	...
	openSelect = function()
		...
		-- Center the window
		local screenW, screenH = guiGetScreenSize()
		local windowW, windowH = guiGetSize(colorPicker.GUI.selectWindow, false)
		local posX, posY = (screenW - windowW) / 2, (screenH - windowH) / 2
		guiSetPosition(colorPicker.GUI.selectWindow, posX, posY, false)
	end,
	...
	updateSelectedValue = function()
		...
		if colorPicker.pickingColor then
			if cursorX < paletteX then
				cursorX = paletteX
			elseif cursorX > paletteX + 255 then
				cursorX = paletteX + 255
			end
			if cursorY < paletteY then
				cursorY = paletteY
			elseif cursorY > paletteY + 255 then
				cursorY = paletteY + 255
			end
		...
		elseif colorPicker.pickingLuminance then
			if cursorY < luminanceY then
				cursorY = luminanceY
			elseif cursorY > luminanceY + 256 then
				cursorY = luminanceY + 256
			end
		...
		elseif colorPicker.pickingAlpha then
			if cursorY < alphaY then
				cursorY = alphaY
			elseif cursorY > alphaY + 255 then
				cursorY = alphaY + 255
			end
	...
	pickColor = function(button, state, cursorX, cursorY)
		...
		minX, maxX = wx + colorPicker.selectWindow.paletteX + 255 + colorPicker.selectWindow.alphaOffset,
			wx + colorPicker.selectWindow.paletteX + 255 + colorPicker.selectWindow.alphaOffset +
			colorPicker.selectWindow.alphaWidth + 12
